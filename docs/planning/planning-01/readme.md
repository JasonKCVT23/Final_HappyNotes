...
1. **卡片操作功能**
    - **摺疊 (Fold)**：將卡片內容折疊以節省空間。
    - **全螢幕顯示 (Full Screen)**：支援卡片全螢幕顯示。
    - **卡片尺寸調整**：根據內容自動調整卡片大小。
    - **側邊欄顯示**：將卡片添加到側邊欄方便瀏覽。
    - **複製/刪除**：可複製或刪除卡片。
    - **移動功能**：卡片可以移動到其他文件或分組。
...
目前想先做 2 和 3 ，先把基礎功能做好再做登入登出。

當然還有更多進階功能，像是 離線編輯、PDF 匯入匯出、卡片連結、卡片標記 、卡片版本管理。

或是思考到**技術的擴展性，像是網路流量或是性能優化，網路流量的部分可以考慮使用 Nginx 來負載平衡，避免單點壓力過大。除此之外，因為我們使用 MongoDB，可以使用分片技術減輕單一資料庫負荷。**
...
像是 coding style 、 branch model 、 trello 專案管理 以前課程都沒有要求也沒有試過，

所以不知道實務上如何運作，有些事情可能要試過才知道如何調整，但我們覺得是個很好讓自己成長的機會，不會才代表我們的成長空間還很多。